/*
 * This is the TypeKit / Google thing, imported with
 * Copy And Paste Package Manager 3000
 */
...
function maptimeLogo(id, text, options) {
    var global = window;
    ...
    function render() {
        ...
        if(text.length<10){
        var fsize = 130 * (width / 700);
      }
      if(text.length>15){
      var fsize = 80 * (width / 700);
    }
      else{
        var fsize = 100 *(width/700)
      }
        var hoff = (width / 700) * 120;
        var offstep = (width / 700) * 5;
        ctx.font = 'normal ' + fsize + 'px Comfortaa';
        var colors = circularQueue([
            '#662d91',
            '#00aeee',
            '#38b54a',
            '#fdf000',
        ...
        var now = +new Date();
        requestInterval(frame);
        frame();

        function frame() {
            if (+new Date() - now < 200) return;
            ctx.clearRect(0, 0, width, height);
            for (var i = 0; i < 6; i++) {
                var off = i * offstep;
                ctx.fillStyle = colors();
                ctx.fillText(text, 10 + off, hoff + off);
            }
            colors();
            now = +new Date();
        }
